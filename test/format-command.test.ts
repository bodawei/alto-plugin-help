...
ARGUMENTS
  ...
  describe('usage', () => {
    test
    .commandHelp(class extends Command {
        static id = 'apps:create' // if you provide your own id, we'll blindly show whatever you specified
  ...
  $ oclif oclif apps:create usage`))
    ...
    test
    .commandHelp(class extends Command {
        static id = 'appsBOGUScreate' // if you provide your own id, we'll blindly show whatever you specified

        static usage = ['<%= config.bin %>', '<%= command.id %> usage']
    })
    .it('outputs usage arrays with templates', (ctx: any) => expect(ctx.commandHelp).to.equal(`USAGE
  $ oclif oclif
  $ oclif appsBOGUScreate usage`))
